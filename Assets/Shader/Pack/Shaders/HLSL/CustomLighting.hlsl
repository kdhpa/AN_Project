...
// Check if we're in HDRP
#if defined(UNITY_SHADER_VARIABLES_INCLUDED) && !defined(UNIVERSAL_LIGHTING_INCLUDED)
    #define IS_HDRP 1
#else
    #define IS_HDRP 0
#endif
...
void MainLight_float(float3 WorldPos, out float3 Direction, out float3 Color, out float DistanceAtten, out float ShadowAtten)
{
    ...
    ShadowAtten = 1;
#elif IS_HDRP
    // HDRP fallback - simplified lighting
...
#if !defined(SHADERGRAPH_PREVIEW) && !IS_HDRP
    Smoothness = exp2(10 * Smoothness + 1);
    WorldNormal = normalize(WorldNormal);
    WorldView = SafeNormalize(WorldView);
